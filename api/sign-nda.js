...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!sessionId.startsWith('cs_')) {
      throw new Error('Nieprawidłowy identyfikator płatności.');
    }
    ...
    if (
      ...
      throw new Error('Uzupełnij wszystkie wymagane dane.');
    }

    if (
      fullName.toLocaleLowerCase('pl') !==
      signature.toLocaleLowerCase('pl')
    ) {
      throw new Error(
        'Podpis musi być identyczny jak imię i nazwisko.'
      );
    }

    if (body.ndaVersion !== NDA_VERSION) {
      throw new Error(
        'Wersja NDA jest nieprawidłowa. Odśwież stronę.'
      );
    }

    if (
      !body.confidentialityAccepted ||
      !body.electronicSignatureAccepted ||
      !body.privacyAccepted
    ) {
      throw new Error(
        'Wszystkie wymagane zgody muszą zostać zaznaczone.'
      );
    }
    ...
    if (!stripeEmail) {
      throw new Error(
        'Sesja Stripe nie zawiera adresu e-mail klienta.'
      );
    }

    const signedAt = new Date().toISOString();

    const ndaTextHash = crypto
      .createHash('sha256')
      .update(NDA_VERSION)
      .digest('hex');
    ...
    await supabaseRequest(
      'nda_signatures?on_conflict=stripe_session_id',
      {
        method: 'POST',
        headers: {
    ...
    return sendJson(res, 400, {
      error: error.message || 'Nie udało się zapisać podpisu.'
    });
  }
};
