(() => {
  ...
  async function apiFetch(url, options = {}) {
    ...
    try {
      ...
      if (error?.name === 'AbortError') throw new Error('Serwer nie odpowiedział na czas. Spróbuj ponownie.');
      throw error;
    } finally {
      clearTimeout(timer);
    }
  ...
  async function verifyPayment() {
    ...
    try {
      const response = await apiFetch(`/api/verify-payment?session_id=${encodeURIComponent(sessionId)}`, {
        headers: { 'Accept': 'application/json' }
      });
      const data = await parseResponse(response);
      verifiedEmail = data.email || '';
      emailInput.value = verifiedEmail;
      const fullNameInput = ndaForm.elements.fullName;
      if (fullNameInput && data.customerName) fullNameInput.value = data.customerName;
      loading.classList.add('hidden');
  ...
  ndaForm.addEventListener('submit', async (event) => {
    ...
    try {
      const response = await apiFetch('/api/sign-nda', {
        method: 'POST',
